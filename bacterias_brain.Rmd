...
# store coefficients
coef_a2 <- data.frame(values = signif(coef(a_q2_fn), 4), data = "dataset A")
...
coef_a3 <- data.frame(values = signif(coef(a_q3_fit), 4), data = "with (0,0)")
coef_comp <- coef_a2 %>% 
  rbind (coef_a3) 

kable(data.frame(values = signif(coef(a_q2_fn), 4)))
...
As we can see, the Mean Standard Error (MSE) for the model fits much better within the range of 10\<= t \<= 15 with a MSE of `r b_q2_mse`, but not well over the entire data set with a MSE of `r b_q2_mse_all`.
