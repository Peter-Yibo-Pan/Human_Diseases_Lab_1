...
We realize that this equation does not fit the initial condition of $z(0) = 0$, hence we adding the term $Ae^{-\tilde{t}}$ with $A$ being a free constant (not to be confused with $A$ from derivation A), which we adjust to $A = 1$ to satisfy this initial condition, resulting in: 
...
Data Set A: 
`r kable(signif(novick_a, 3))`

Data Set B: 
`r kable(signif(novick_b, 3))`
