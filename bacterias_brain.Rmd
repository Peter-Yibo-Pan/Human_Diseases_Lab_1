...
While attempting to fit data set A's function ($z(t) = A(1 - e^{-\frac{t}{\tau}})$) over data set B, we realize that it is impossible to fit. If we were to fit the function around the range $t \le 15$, the data point would inevitably exceed the value for $A$, which is impossible since $A$ defines the theoretical maximum amount of \beta-gal that can exist in the system, as it is defined as $\frac{NSV}{Q}$. Hence the amount of \beta-gal should only asymptotically approach the value for $A$. If we try to find a value for $A$, the value would be unreasonably high to the point where the function is irrelevant to the data (See Figure 5). 
